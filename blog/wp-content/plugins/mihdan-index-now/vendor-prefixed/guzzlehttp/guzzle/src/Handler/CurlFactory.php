...
use Mihdan\IndexNow\Dependencies\GuzzleHttp\Psr7\LazyOpenStream;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    public function create(RequestInterface $request, array $options) : EasyHandle
    {
        ...
        if ('' === $protocolVersion) {
            ...
            $protocolVersion = '1.1';
            $request = \Mihdan\IndexNow\Dependencies\GuzzleHttp\Psr7\Utils::modifyRequest($request, ['version' => $protocolVersion]);
        }
    ...
    private static function triggerUnsupportedRequestOptionDeprecations(array $options) : void
    {
        if (\array_key_exists('stream_context', $options)) {
            \Mihdan\IndexNow\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "stream_context" request option to a cURL handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because cURL handlers ignore PHP stream context options.');
        }
    }
    /**
    ...
    private static function createRejection(EasyHandle $easy, array $ctx) : PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \Mihdan\IndexNow\Dependencies\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri) : string
    {
        ...
        $redactedUriString = \Mihdan\IndexNow\Dependencies\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    {
        $mask = 0;
        if (\in_array('http', $protocols, \true)) {
            $mask |= \CURLPROTO_HTTP;
        }
        if (\in_array('https', $protocols, \true)) {
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \Mihdan\IndexNow\Dependencies\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \Mihdan\IndexNow\Dependencies\GuzzleHttp\Psr7\Utils::streamFor($sink);
        ...
        if (isset($options['proxy'])) {
            if (!\is_array($options['proxy'])) {
                ...
                if (isset($options['proxy'][$scheme])) {
                    if (isset($options['proxy']['no']) && Utils::isUriInNoProxy($easy->request->getUri(), $options['proxy']['no'])) {
