...
use Mihdan\IndexNow\Dependencies\GuzzleHttp\Exception\ConnectException;
...
use Mihdan\IndexNow\Dependencies\GuzzleHttp\Promise\FulfilledPromise;
use Mihdan\IndexNow\Dependencies\GuzzleHttp\Promise\PromiseInterface;
use Mihdan\IndexNow\Dependencies\GuzzleHttp\Psr7;
use Mihdan\IndexNow\Dependencies\GuzzleHttp\TransferStats;
...
class StreamHandler
{
    ...
    private function createStream(RequestInterface $request, array $options)
    {
        ...
        return $this->createResource(function () use($uri, $contextResource, $context, $options, $request) {
            ...
            if (\function_exists('Mihdan\\IndexNow\\Dependencies\\http_get_last_response_headers')) {
                $http_response_header = \Mihdan\IndexNow\Dependencies\http_get_last_response_headers();
            }
    ...
    private static function triggerUnsupportedRequestOptionDeprecations(RequestInterface $request, array $options) : void
    {
        if (\array_key_exists('curl', $options) && $options['curl'] !== null && $options['curl'] !== [] && !self::isCurlOptionGeneratedByAuth($options)) {
            \Mihdan\IndexNow\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "curl" request option to the stream handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because the stream handler ignores cURL options.');
        }
        if (self::usesDigestAuth($options)) {
            \Mihdan\IndexNow\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing digest authentication to the stream handler is deprecated; guzzlehttp/guzzle 8.0 will reject digest authentication with the stream handler because it is only supported by cURL handlers.');
        }
        if (\array_key_exists('expect', $options) && $options['expect'] !== \false && $request->hasHeader('Expect')) {
            \Mihdan\IndexNow\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "expect" request option to the stream handler is deprecated when it adds an Expect header; guzzlehttp/guzzle 8.0 will reject this option because the stream handler does not support Expect: 100-Continue.');
        }
    }
    ...
    {
        return isset($options['auth'][2]) && \is_string($options['auth'][2]) && \strtolower($options['auth'][2]) === 'digest';
    }
    /**
