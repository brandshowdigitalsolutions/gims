...
use Mihdan\IndexNow\Dependencies\GuzzleHttp\Handler\CurlHandler;
use Mihdan\IndexNow\Dependencies\GuzzleHttp\Handler\CurlMultiHandler;
use Mihdan\IndexNow\Dependencies\GuzzleHttp\Handler\CurlShareHandleState;
use Mihdan\IndexNow\Dependencies\GuzzleHttp\Handler\Proxy;
use Mihdan\IndexNow\Dependencies\GuzzleHttp\Handler\StreamHandler;
...
final class Utils
{
    ...
    public static function chooseHandler(array $handlerOptions = []) : callable
    {
        ...
        $curlHandlerOptions = [];
        $curlSupported = \defined('CURLOPT_CUSTOMREQUEST') && \function_exists('curl_version') && \version_compare(\curl_version()['version'], '7.21.2') >= 0 && (\function_exists('curl_multi_exec') || \function_exists('curl_exec'));
        ...
        if (\ini_get('allow_url_fopen')) {
            ...
            $handler = $handler ? Proxy::wrapStreaming($handler, $streamHandler) : $streamHandler;
        } elseif (!$handler) {
...
EOT
    ...
    public static function isHostInNoProxy(string $host, array $noProxyArray) : bool
    {
        ...
        $host = self::normalizeNoProxyHost($host, \true);
        foreach ($noProxyArray as $area) {
            ...
            $area = '.' . \ltrim($area, '.');
            if (\strpos($host, ':') === \false && \strpos($area, ':') === \false && \substr($host, -\strlen($area)) === $area) {
                return \true;
            }
        }
        return \false;
    ...
    private static function normalizeNoProxyHost(string $host, bool $stripPort) : string
    {
        if ($host !== '' && $host[0] === '[') {
            ...
            if ($closingBracket !== \false) {
                ...
                if (($tail === '' || $stripPort && \preg_match('/^:\\d+$/', $tail)) && \filter_var($address, \FILTER_VALIDATE_IP, \FILTER_FLAG_IPV6)) {
    ...
    public static function normalizeIdnConversionOption($value) : ?int
    {
        ...
        if (\is_string($value) && \is_numeric($value) || \is_float($value) && \is_finite($value)) {
            \Mihdan\IndexNow\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing %s as the "idn_conversion" request option is deprecated; guzzlehttp/guzzle 8.0 will reject values that are not true, false, null, or an integer IDNA_* bitmask.', self::describeType($value));
            return (int) $value;
        }
