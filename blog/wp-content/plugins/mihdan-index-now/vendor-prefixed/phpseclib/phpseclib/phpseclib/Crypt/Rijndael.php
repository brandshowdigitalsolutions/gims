...
use Mihdan\IndexNow\Dependencies\phpseclib3\Crypt\Common\BlockCipher;
use Mihdan\IndexNow\Dependencies\phpseclib3\Exception\BadDecryptionException;
use Mihdan\IndexNow\Dependencies\phpseclib3\Exception\BadModeException;
use Mihdan\IndexNow\Dependencies\phpseclib3\Exception\InconsistentSetupException;
...
class Rijndael extends BlockCipher
{
    ...
    protected function decryptBlock($in)
    {
        ...
        $l = $Nb - $c[3];
        while ($i < $Nb) {
            $word = $state[$i] & (\PHP_INT_SIZE == 8 ? 0xff000000 : -16777216) | $state[$j] & 0xff0000 | $state[$k] & 0xff00 | $state[$l] & 0xff;
    ...
    protected function setupKey()
    {
        ...
        for ($i = $this->Nk; $i < $length; $i++) {
            ...
            if ($i % $this->Nk == 0) {
                ...
                $temp = \PHP_INT_SIZE == 8 ? $temp << 8 & 0xffffff00 | $temp >> 24 & 0xff : $temp << 8 | $temp >> 24 & 0xff;
    ...
    protected function &getTables()
    {
        ...
        if (empty($tables)) {
            ...
            foreach ($t3 as $t3i) {
                $t0[] = $t3i << 24 & self::safe_intval(0xff000000) | $t3i >> 8 & 0xffffff;
                $t1[] = $t3i << 16 & self::safe_intval(0xffff0000) | $t3i >> 16 & 0xffff;
                $t2[] = $t3i << 8 & self::safe_intval(0xffffff00) | $t3i >> 24 & 0xff;
            }
            $tables = [
        ...
        if (empty($tables)) {
            ...
            if (\PHP_INT_SIZE === 8) {
                foreach ($dt3 as $dt3i) {
                    ...
                    $dt1[] = $dt3i << 16 & 0xffff0000 | $dt3i >> 16 & 0xffff;
                    $dt2[] = $dt3i << 8 & 0xffffff00 | $dt3i >> 24 & 0xff;
                }
            } else {
                foreach ($dt3 as $dt3i) {
                    $dt0[] = $dt3i << 24 | $dt3i >> 8 & 0xffffff;
                    $dt1[] = $dt3i << 16 | $dt3i >> 16 & 0xffff;
                    $dt2[] = $dt3i << 8 | $dt3i >> 24 & 0xff;
                }
            }
            $tables = [
