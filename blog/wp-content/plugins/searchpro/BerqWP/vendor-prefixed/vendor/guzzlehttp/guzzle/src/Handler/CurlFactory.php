...
namespace BerqWP_Deps\GuzzleHttp\Handler;

use BerqWP_Deps\GuzzleHttp\Exception\ConnectException;
use BerqWP_Deps\GuzzleHttp\Exception\RequestException;
use BerqWP_Deps\GuzzleHttp\Promise as P;
use BerqWP_Deps\GuzzleHttp\Promise\FulfilledPromise;
use BerqWP_Deps\GuzzleHttp\Promise\PromiseInterface;
use BerqWP_Deps\GuzzleHttp\Psr7\LazyOpenStream;
use BerqWP_Deps\GuzzleHttp\TransferStats;
use BerqWP_Deps\GuzzleHttp\Utils;
use BerqWP_Deps\Psr\Http\Message\RequestInterface;
use BerqWP_Deps\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \BerqWP_Deps\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \BerqWP_Deps\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \BerqWP_Deps\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \BerqWP_Deps\GuzzleHttp\Psr7\Utils::streamFor($sink);
