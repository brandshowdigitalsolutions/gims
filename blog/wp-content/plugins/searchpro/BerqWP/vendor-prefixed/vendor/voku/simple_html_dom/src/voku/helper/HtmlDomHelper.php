...
final class HtmlDomHelper
{
     ...
     * @return string
     */
    public static function mergeHtmlAttributes(string $html, string $optionStr, string $htmlCssSelector): string
    {
        if (!$optionStr) {
            return $html;
        }
        $dom = \BerqWP_Deps\voku\helper\HtmlDomParser::str_get_html($html);
        $domNew = \BerqWP_Deps\voku\helper\HtmlDomParser::str_get_html('<textarea ' . $optionStr . '></textarea>');
        ...
        foreach ($attributesNew as $attributeNameNew => $attributeValueNew) {
            ...
            if ($attributeNameNew === 'class' || $attributeNameNew === 'style' || \strpos($attributeNameNew, 'on') === 0) {
        ...
        foreach ($attributes as $attributeName => $attributeValue) {
            $domElement->setAttribute($attributeName, $attributeValue, \true);
        }
