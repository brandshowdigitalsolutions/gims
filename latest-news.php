...
<head>
	...
	<meta name="description" content="Latest News - GNIOT Institute of Management Studies One of The Best PGDM Campus and Top College for PGDM provides Best PGDM private Colleges in Delhi, India."/>
            ...
            <div class="intro">
                <div class="container custom_container">
                    <div class="row align-items-center new-pd4 zoomanimate">
						...
						<div class="col-lg-12">
							<h1 class="page-t">Latest News - GIMS</h1>
							<h3 class="sub-t">PGDM College - Greater Noida</h3>
						</div>
							...
							<div class="row">
								<div class="col-lg-9">
									<div class="col-lg-12 new-pd9 awardbg2">
										 <?php 
											global $conn;
											$sql=mysqli_query($conn,"select * from tbl_latest_news ORDER BY STR_TO_DATE(date, '%m/%d/%Y') DESC LIMIT $start_from, $limit");
											while($row=mysqli_fetch_array($sql)){
										?>
										<div class="single-item line-b">
											<div class="item-img">
												<?php $imgex=explode(",",$row['images']);?>
												<a href="<?php echo page_link();?>latest-news-detailed.php?slug=<?php echo $row['newsurl']; ?>"<img src="<?php echo page_link();?>latestnews/<?php echo $imgex[0];?>" alt="event" class="img-responsive"></a>
											</div>
											<div class="item-content">
												<h3 class="sidebar-title"><a href="<?php echo page_link();?>latest-news-detailed.php?slug=<?php echo $row['newsurl']; ?>"><?php echo $row['title'];?></a></h3>
												<ul class="event-info-block">
													<li><i class="icon-calendar" aria-hidden="true"></i> <?php $datex=explode(",",date("j F, Y", strtotime($row["date"])));echo $datex[0]."".$datex[1]?></li>
													<li><i class="icon-map-marker" aria-hidden="true"></i> <?php echo $row['location'];?></li>
												</ul>
												<p class="no-padding"><?php echo $row['sdescription'];?></p>
												<a href="<?php echo page_link();?>latest-news-detailed.php?slug=<?php echo $row['newsurl']; ?>" class="newsbutton">Read More <i class="icon-long-arrow-right"></i></a>
											</div>
										</div>
										<?php } ?>
										<ul class="pagination-center">
											<?php   
												$sql = "SELECT COUNT(*) FROM tbl_latest_news";   
